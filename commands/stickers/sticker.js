    ...
    try {
      if (args[0] === '-list') {
        let helpText = `📻 🎙️ *𝗖𝗔𝗧𝗔𝗟𝗢𝗚𝗢 𝗗𝗘 𝗘𝗦𝗧𝗔𝗠𝗣𝗔𝗗𝗢𝗦* 🎙️ 📻\n\n` +
          ...
          `➔ *-grayscale* : Televisión en Blanco y Negro\n` +
          `➔ *-invert* : El Lado Oscuro\n` +
          `➔ *-rotate90* : Giro de Guion\n\n` +
      ...
      const name = user.name || 'Espectador';
      
      // Personalización del Pack/Autor
      const meta1 = user.metadatos ? String(user.metadatos).trim() : '';
      const meta2 = user.metadatos2 ? String(user.metadatos2).trim() : '';
      let texto1 = meta1 ? meta1 : `📻 𝖱𝖺𝖽𝗂𝗈 𝖣𝖾𝗆𝗈𝗇 𝖲𝗍𝗂𝖼𝗄𝖾𝗋𝗌`;
      let texto2 = meta2 ? meta2 : `🎙️ 𝖠𝗅𝖺𝗌𝗍𝗈𝗋 - ${name}`;
      ...
      return m.reply(`📻 *¡CRASH!* La estática nos invade... \n> [Error: *${e.message}*]\n¡Sonríe, esto es solo un bache! ♪`);
    }
  }
};

// ... (isUrl y buildFFmpegFilters se mantienen igual, son la maquinaria pesada)
...
const buildFFmpegFilters = (effects, isVideo = false) => {
  ...
  for (const effect of effectList) {
    ...
    if (effect === 'blur') filters.push('gblur=sigma=5');
