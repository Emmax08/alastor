...
export default async (client, m) => {
  ...
  if (!(command === 'invite')) {
    ...
    const userName = global.db.data.users[m.sender]?.name || 'Pecador'
    
    // Mensaje de advertencia y expulsión temática
    await client.reply(m.chat, `> ⚠️ **Has roto las reglas y serás exterminado por los ángeles.**\n\nAdios *${userName}*, no se permiten enlaces de ${isChannelLink ? 'canales' : 'otros grupos'} en este inframundo.`, null)
