export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    // Obtenemos los datos directamente desde el objeto global db
    ...
    // Verificación de economía activada
    ...
    // Verificación de tiempo de espera (Cooldown)
    ...
    if (rand < 0.4) {
      ...
      // EVENTO FALLIDO (Pierde monedas)
      cantidad = Math.floor(Math.random() * (11000 - 9000 + 1)) + 9000
      const total = (user.coins || 0) + (user.bank || 0)
      
      // Restar de la billetera o el banco si es necesario
      if (total >= cantidad) {
      ...
      // EVENTO NEUTRAL (Sin cambios económicos)
    ...
    // Actualizamos el tiempo de la última aventura (20 minutos de cooldown)
